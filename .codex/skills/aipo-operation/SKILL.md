...
```bash
python3 .codex/skills/aipo-operation/scripts/weekly_review.py --path "<layer_dir>" --lang <ja|en>
```

`programs/<project_name>/` を直接指定する場合:

```bash
python3 .codex/skills/aipo-operation/scripts/weekly_review.py --project "<project_name>" --lang <ja|en>
```

## 出力

- デフォルト出力先: `<layer_dir>/weekly_review/weekly_review_YYYY-MM-DD.md`
- 内容仕様: `.codex/skills/aipo-operation/references/report-format.md`
...
- ユーザーが `lang: ja|en` のように指定した場合はそれを優先する（ユーザー指定が無い場合は **`--lang auto` に依存しない**）。
