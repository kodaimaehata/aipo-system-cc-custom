...
- `flows/` 配下の会話ベースの調査メモ、整理ノート、下書き成果物も有効なコンテキスト源として扱う。
- 類似知見の収集対象は `programs/archived_projects.md` だけに限定しない。`programs/` 配下の**進行中プロジェクト / SubLayer** も確認し、再利用価値のある成果物や意思決定を取り込む。
- 市場、規制、競合、製品仕様、価格、技術仕様など**時間変化しうる外部情報**は、必要に応じて `web_search` で補完する。
...
- 入力（必要な情報・ファイル）
- 手順（コマンド例を含む）
- 成果物は、同一レイヤーで共有する主成果物ファイル1つを更新し、`T0xx` ごとに章を分ける
- リスク/注意点
