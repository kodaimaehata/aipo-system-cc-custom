...
def _render_command_md(task: dict) -> str:
    ...
    return (
        ...
        "- （完了条件を箇条書きで）\n"
        f"- （共通成果物ファイル内の `## {task_id} {task_name}` 章が更新されている）\n\n"
        "## Inputs\n"
        "- （必要な前提情報・参照ファイル・URLなど）\n"
        "- （共通成果物ファイルのパス。未決なら最初に1つ決めて以後固定する）\n\n"
        "## Steps\n"
        "1. 共通成果物ファイルを確認する。未作成ならファイル名を1つ決めて新規作成する。\n"
        f"2. 共通成果物ファイル内に `## {task_id} {task_name}` 章を作成し、このタスクの内容を追記・更新する。\n"
        "3. 必要なら前タスクの章を参照し、重複を避けつつ整合を取る。\n\n"
        "## Outputs\n"
        "- （共通成果物ファイル1つ。例: `documents/layer_deliverable.md`）\n"
        f"- （上記ファイル内の `## {task_id} {task_name}` 章）\n\n"
        "## Notes\n"
        "- 成果物はタスクごとに別ファイルを増やさず、同一レイヤー内の主成果物ファイルへ章追加する。\n"
        f"{template_line}\n"
    )
