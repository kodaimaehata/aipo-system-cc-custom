...
| ID | 名称 | 説明 |
|----|------|------|
| `local_workspace` | ローカル検索（ワークスペース） | ワークスペース内のGoal関連情報を検索。`src/` `docs/` に加え、`flows/` の作業ログやメモ、`mini-projects/` の小規模開発資産も必要に応じて含める |
...
- `mini-projects/` は Git 管理外でも保持される小規模開発の brief / requirements / design / plan / review 資産の置き場として扱い、AIPO に昇格していない類似ツール・PoC の知見があれば参照候補へ含める。
