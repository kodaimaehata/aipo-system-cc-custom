...
【制約・リソース確認】
- <question_constraint_1>
- <question_constraint_2>
...
- 「オープンクエスチョン」は以下の3カテゴリから生成する：
  - **ゴール明確化**: 成功基準の曖昧さ、測定方法の不明点、優先度
  - **制約・リソース確認**: 技術的制約、利用可能リソース、期限、予算
  - **スコープ確認**: 対象範囲、除外事項、依存関係
- 収集したコンテキストで不足・曖昧な点を質問として提示する。
- 各カテゴリ最低1つ、合計3〜7個程度を目安とする。
- 質問がない場合は「現時点で追加確認事項はありません」と記載する。
